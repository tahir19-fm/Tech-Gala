...
class EventRegistrationActivity : AppCompatActivity(), PaymentResultListener {
    ...
    private fun openEventRegisterFragment() {
        supportFragmentManager.beginTransaction()
            .replace(R.id.fragmentContainer, RegisterForEventFragment()).commit()
    }

    private fun openEventDetailFragment() {
        supportFragmentManager.beginTransaction()
            .replace(R.id.fragmentContainer, EventDetailsFragment()).commit()
    }
    ...
    private fun registerUser() {
        ...
        db.collection("eventsregistered").document(id).collection("events")
            .document(viewModel.userId.value.toString())
            .set(user)
